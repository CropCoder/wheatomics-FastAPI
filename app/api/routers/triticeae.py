...
@router.get("/papers")
def search_papers(
    q: str | None = Query(None, description="全文关键词搜索（paper_title / abstract / authors）", example="CRISPR"),
    gene_name: str | None = Query(None, description="基因名称关键词", example="TaCPK-1"),
    trait_label: str | None = Query(None, description="性状标签精确匹配"),
    evidence_type: str | None = Query(None, description="证据类型精确匹配", example="seed_title_match"),
    min_confidence: float | None = Query(None, ge=0, le=1, description="最低置信度", example=0.9),
    review_status: str | None = Query(None, description="审核状态", example="auto_pass"),
    ai_tags: str | None = Query(None, description="AI 标签关键词", example="stripe_rust"),
    functional_gene_tags: str | None = Query(None, description="功能基因标签关键词", example="functional gene"),
    pubmed_keywords: str | None = Query(None, description="PubMed 关键词"),
    new_tags: str | None = Query(None, description="新标签关键词"),
    pub_date_start: str | None = Query(None, description="发布日期范围起始（年份）", example="2024"),
